...
第 32 章把物理量翻译成Hilbert 空间上的自伴算子，把测量结果翻译成算子的谱。那一步给出了整套语言的骨架，却没有给出任何一个具体的数。本章给这副骨架上第一台能运转的发动机：**Schrödinger 方程**。
...
与前后章的关系：第 32 章给了算子与谱，本章给方程与第一个可解模型，并把"用算子代数代替微分方程"这条技术线第一次走通。本章末尾会撞上一个技术裂缝：$$a$$、$$a^\dagger$$、$$X$$、$$P$$ 都不可能是有界算子，"谱"与"定义域"必须在更严格的意义下重新谈。那正是第 36 章的主题。
...
按第 32 章的规则量子化：位置 $$x\mapsto X$$（"乘 $$x$$"的算子），动量 $$p\mapsto P = -i\hbar\frac{d}{dx}$$，两者满足正则对易关系 $$[X,P] = i\hbar I$$。请只用手边这两条信息回答下列问题：
...
第 32 章结尾留下的悬念（可观测量、本征态、测量结果）在这里第一次被真正用来算出一个数。
...
**定义 3.1（Schrödinger 方程与时间演化算子, Schrödinger equation）**。设 $$\mathscr H = L^2(\Omega)$$ 是第 30 章构造的 Hilbert 空间，$$H$$ 是 $$\mathscr H$$ 上的自伴算子（第 32 章的可观测量）。**含时 Schrödinger 方程**是
...
即每个有限部分和都是解；无穷和取极限仍是解（这一步用到 $$\psi_0\in\operatorname{Dom}H$$ 与 $$H$$ 的闭性，详见第 42 章）。再对 $$t=0$$ 的条件
...
**定理 3.4 说**：Schrödinger 方程的求解被归结为两件互不牵涉的事——**(甲) 求 $$H$$ 的本征值与本征函数；(乙) 把初值在这个本征函数系上展开**。这就是第 32 章那句"量子力学的计算总是求算子的谱"的严格出处。

**解说 3.1（谱连续时怎么办）**。定理 3.4 假定了本征值可排成一列。若 $$H$$ 的谱里出现连续段（如自由粒子的 $$H=-\frac{\hbar^2}{2m}\partial_x^2$$），求和要换成对谱测度的积分，$$\lvert c_k\rvert^2$$ 相应地换成"谱密度"。这一步的准确陈述需要投影算子值测度，见第 38 章；本章只处理谱离散的谐振子，其中级数型的定理 3.4 完全够用。
...
分别称为**位置算子**与**动量算子**（定义 3.5 与定义 3.6，第 32 章），它们满足**正则对易关系 (canonical commutation relation)**
...
（$$H$$ 就是经典 Hamilton 量 $$H_{\mathrm{cl}}=\frac{p^2}{2m}+\frac{m\omega^2}{2}x^2$$ 把 $$x,p$$ 换成算子的结果；第 32 章的对应规则 $$F\mapsto F(X,P)$$ 在此第一次被用于构造具体模型。）
...
它们不交换。按 Leipniz 律（定理 3.8，第 32 章）展开左端，只用 $$[X,P]=i\hbar I$$：
...
（$$a$$ 与 $$a^\dagger$$ 的"伴随"一词见定义 3.5，第 32 章；本节的 $$X,P$$ 都是自伴的，故把 $$P$$ 换成 $$-P$$ 即得伴随。）
...
**定理 3.8 是入口题 (ii) 的答案。** 注意它的分量：一个含 $$X$$ 与 $$P$$ 的二次表达式，被换成了**一个算子的乘积**。这个替换之所以可能，恰恰因为那个"差项"是纯量；若 $$[X,P]$$ 不是纯量，$$H$$ 就不是任何 $$a^\dagger a$$ 的倍数（这一点在第 44 章会用表示的不可约性说得更准）。
...
**一点评注**。定理 3.15 的证明实际用到的只是"位置与动量不能同时取 0"，而这正是正则对易关系 $$[X,P]=i\hbar I$$ 的直接后果（第 32 章的定理 3.7）：若 $$P\psi=X\psi=0$$，则由 $$[X,P]\psi = (XP-PX)\psi = 0$$ 得 $$i\hbar\psi=0$$，$$\psi=0$$。所以**零点能的存在性等价于正则对易关系的非平凡性**——这不是计算方法的问题，是代数结构的必然。物理上它就是 Heisenberg 不确定性原理在极低温下的残余。
...
*证明思路*：由 $$\psi_n=H_ne^{-x^2/2}$$，$$\{\psi_n\}$$ 是自伴算子 $$N$$ 属于互异本征值的本征函数。互异本征值的本征函数正交（第 32 章定理 3.11），代进去即得。剩下的常数用 $$h_n$$ 的首项系数 $$2^n$$ 与 $$\int h_n^2e^{-x^2}$$ 的显式公式算出。
...
这里有一个容易忽略的要点：**"能谱等间距"并不是因为势能是抛物线**，而是因为 $$[N,a]=-a$$ 右端的系数恰好是 $$-1$$（这是入口题 (vi) 的答案）。若对易子写成 $$[N,a]=-ca$$，梯子的步长就是 $$c$$，能谱变成等差数列。谐振子之所以特别好，是因为它对应的梯子**每级等高**。凡是有这种"每级等高"的梯子，代数方法就有效——角动量的 $$J_\pm$$（第 52 章）、氢原子的 $$SO(4)$$ 升降（第 14 章知识库）都是同一套技术。
...
第 32 章末尾留下的"谱是什么"这一问题，在本章得到了第一个可以算出来的答案：谱就是那条梯子上的刻度。
...
**考点**：内积与期望值（第 32 章定理 3.11）。**位置**：定理 3.3 后面那句"平稳态的概率密度不随时间变化"的具体实例。
...
**考点**：推论 3.12 与量子-经典对应的极限。**位置**：把本章结果接到第 18 章的准经典近似。
...
**研1.**（算子的无界性）证明谐振子的 $$X,P,H,N$$ 都不是有界算子；并说明为什么前面所有关于"本征值""本征向量"的讨论都必须附加"定义域"这一层。研究档提示：这正是第 36 章的入口。

**研2.**（升降机制的 Lie 代数读法）考虑两个算子

$$K_+ = \frac{i}{2\hbar}P+\frac{m\omega}{2\hbar}X,\qquad K_- = \frac{i}{2\hbar}P-\frac{m\omega}{2\hbar}X .$$

求 $$[K_+,K_-]$$ 与 $$[H,K_\pm]$$，说明 $$\{K_+,K_-,H\}$$ 张成怎样一个 Lie 代数，并指出本章的升降机制在这个代数里对应什么。研究档提示：这个对象在第 44 章（正则对易关系与 Stone–von Neumann）与第 38–39 章（Lorentz 群的表示论）会以完整形态回来。
...
**(iii) $$E\in\mathbb R$$ 与 $$T(t)$$。** 由第 32 章 $$H$$ 自伴，把 $$H$$ 从内积的一侧挪到另一侧只取复共轭：
...
**第二步：求和。** 由 $$e^{\lambda a^\dagger}=\sum_{n\ge0}\frac{\lambda^n}{n!}(a^\dagger)^n$$（在适当的收敛意义下，见第 42 章），
...
**这正是第 36 章的起点**：无界算子的谱、自伴与对称的差别、定义域对谱的影响——这些不是技术洁癖，而是 §3.6 里"$$a\psi$$ 可能为零"这类现象的深层原因。$$\blacksquare$$
...
**这是同一件事的第一次露面**：第 44 章要证 Stone–von Neumann 定理，那里的主角 Weyl 代数正是 $$\{X,P,I\}$$ 的中心扩张，$$[X,P]=i\hbar I$$ 的那个 $$I$$ 与上面的中心项是同一回事；第 38–39 章在 Lorentz 群的表示论里会再次遇到升降机制——那时 $$K_\pm$$ 的角色由角动量的 $$J_\pm=J_1\pm iJ_2$$ 扮演、$$H$$ 由 $$J_3$$ 扮演，"能级梯子"变成"磁量子数的梯子"。

**最后走回本章的那道裂缝**：以上全是"形式"对易子演算，而 $$K_\pm,H$$ 都是无界算子——$$a,a^\dagger$$ 究竟定义在哪个子空间上、这个代数与它的"表示""谱"该怎么严格定义，都还没有答案。先把这些在有界算子的框架里说清楚，正是第 36 章（有界算子与算子的谱）的起点。$$\blacksquare$$
...
**5. 同一个代数会以不同面貌反复出现。** $$a,a^\dagger,N$$ 满足的 $$[N,a]=-a$$、$$[N,a^\dagger]=a^\dagger$$ 正是 $$\mathfrak{sl}_2$$ 的权空间机制（研2）。这一结构在第 44 章（Weyl 代数与 Stone–von Neumann）与第 38–39 章（Lorentz 群的表示论）会完整回来。
...
本章靠"默认取最大自然定义域"绕过了这些。**第 36 章要做的是把这层缺口正式补上**：有界算子为什么好、无界算子的谱如何定义、对称与自伴的差别会怎样改变物理结论。从那里出发，第 38 章给出谱定理的准确形式，第 42 章处理无界算子的 Cayley 变换，直到第 44 章才把本章的 $$[X,P]=i\hbar I$$ 放在一个自伴的、唯一性可证的对象上（Weyl 代数与 Stone–von Neumann 定理）。
