...
读完第 79 章的具体例子后——转置矩阵怎么把箭头反过来、一个小链复形怎么手算出 $$\partial^2=0$$、一个 $$2\times2$$ 矩阵的谱怎么用预解式算出来、$$\mathbb Q(\sqrt2)/\mathbb Q$$ 的 Galois 对应怎么只有两条边——这里把同样的构造写成一般定义，并给出完整证明。本章不引入新领地。前 39 章每一章攻一座山；这一章站在山顶，把那 39 章画成一张图。
...
**为什么要定义"反过来的范畴"。** 第 79 章用矩阵转置具体算过：把 $$f:V\to W$$ 换成 $$f^T:W^*\to V^*$$，箭头掉头，而"先 $$f$$ 后 $$g$$"变成"先 $$g^T$$ 后 $$f^T$$"。这个"掉头"操作在向量空间范畴 $$\mathbf{Vect}_k$$ 里用了转置，但它其实只用到了范畴的公理本身，与"向量空间"这个具体身份无关。于是很自然地问：能不能把"掉头"做成对**任意**范畴都成立的操作，使一条陈述的证明也能跟着"掉头"，不用重证？这正是定义 3.1 与定理 3.1 要回答的问题。
...
**为什么条件是 $$\partial^2=0$$，而不是别的。** 第 79 章手算过一个具体三角形（三个顶点、三条边、一个面）的边缘算子：把面的边界拆成三条有向边，再把每条边的边界拆成"终点减起点"，两步合起来每个顶点恰好被加、减各一次，正负抵消为零。这个"抵消"不是巧合——只要"边界的边界"这件事被写成一个公理 $$\partial^2=0$$，"闭而不是边界的东西"（第 79 章手数出来的"洞"）就可以被**除法**——商掉边界——精确地量出来。定义 3.2 把这件事写成对任意环上模的一般陈述。
...
**为什么"谱"要定义成"不可逆的点"，而不是"特征值"。** 有限维时二者是一回事：第 79 章对一个 $$2\times2$$ 矩阵直接解 $$\det(\lambda I-T)=0$$ 得到特征值，再手算过预解式 $$(\lambda I-T)^{-1}$$ 与恒等式 $$R(\lambda)-R(\mu)=(\mu-\lambda)R(\lambda)R(\mu)$$。但在无穷维的 Hilbert/Banach 空间上，"特征多项式"这件事根本不存在（没有行列式），唯一还能问的问题是"$$\lambda I-T$$ 能不能求逆"——这就是定义 3.3 把"谱"定义成不可逆点集合的原因：它是唯一在无穷维还讲得通的版本，而第 79 章的预解恒等式将会是证明"谱非空"的关键代数工具。
...
**为什么"中间域"和"子群"要放进同一个抽象框架里。** 第 79 章对 $$K=\mathbb Q(\sqrt2)/\mathbb Q$$ 手算过全部数据：只有两个中间域（$$\mathbb Q$$、$$K$$）、只有两个子群（$$\{1\}$$、$$G$$），而且"域越大，固定它的自同构越少"这条规律在两个元素的情形下平凡地成立。第 78 章已经证明了一般的"中间域 ↔ 子群"对应，但那条证明混杂着域论的具体内容（自同构、固定点）。这里要做的是把"越大越……越小"这条规律里**域论无关**的部分单独提出来——它只用到"两个偏序集之间有一对互相牵制、方向相反的映射"这一条抽象性质，与"是不是域"完全无关。这样提出来的抽象版本，套到覆叠空间的子群上会重新证一遍完全相同的定理（注 3.4）。
...
(ii) 对任意 $$x$$，由 (i) 的后一式（取 $$y=F(x)$$）得 $$F(G(F(x)))\ge F(x)$$；又由 (i) 的前一式 $$x\le G(F(x))$$，两边作用反序的 $$F$$ 得 $$F(x)\ge F(G(F(x)))$$。两式合起来给出 $$F(G(F(x)))=F(x)$$，即 $$F\circ G\circ F=F$$。

**（展开：对称地得到 $$G\circ F\circ G=G$$）** 把上面的论证里 $$P,Q$$ 与 $$F,G$$ 的角色互换：对任意 $$y\in Q$$，先在 (i) 的前一式 $$x\le G(F(x))$$ 中取 $$x=G(y)$$，得
$$G(y)\le G\big(F(G(y))\big);$$
再在 (i) 的后一式中直接取 $$y$$，得 $$F(G(y))\ge y$$，两边作用反序的 $$G$$ 得 $$G(F(G(y)))\le G(y)$$。两式合起来给出 $$G(F(G(y)))=G(y)$$，即 $$G\circ F\circ G=G$$。

(iii) 先算 $$P_0$$。若 $$x\in P_0$$，即 $$x=G(F(x))$$，则 $$x\in G(Q)$$。反之设 $$x=G(y)$$，由 (ii) 得
$$G(F(x))=G(F(G(y)))=G(y)=x,$$
故 $$x\in P_0$$。于是 $$P_0=G(Q)$$。

**（展开：对称地得到 $$Q_0=F(P)$$）** 若 $$y\in Q_0$$，即 $$y=F(G(y))$$，则 $$y\in F(P)$$。反之设 $$y=F(x)$$，由 (ii) 得
$$F(G(y))=F(G(F(x)))=F(x)=y,$$
故 $$y\in Q_0$$。于是 $$Q_0=F(P)$$。
...
- **Galois 理论**（第 38、39 章）。设 $$k\subseteq K$$ 是**有限** Galois 扩张。令 $$P$$ 为中间域 $$\{E:k\subseteq E\subseteq K\}$$ 按包含序，$$Q$$ 为子群 $$\{H:H\le\operatorname{Gal}(K/k)\}$$ **也**按（标准）包含序——**注意这里不能把 $$Q$$ 的序反过来定义**：定义 3.4 要求的是 $$F,G$$ 本身是反序**映射**，两个偏序集各自仍用自己最自然的序；若强行把 $$Q$$ 也换成反包含序，$$F,G$$ 就变成保序映射，等价式 (iii) 反而会对不上（可以直接代入精讲 4 的表格验证：$$\{1\}\subseteq\langle\sigma\rangle$$ 而 $$F(\mathbb Q)=G\supseteq F\big(\mathbb Q(\sqrt3)\big)=\langle\sigma\rangle$$，这只在 $$Q$$ 取标准包含序时才是"反序"）。取
...
- **覆叠理论**（第 08、25 章）。把 $$P$$ 换成连通覆盖空间（按"谁覆盖谁"排序）、$$Q$$ 换成基本群的子群（同样按标准包含序），$$F$$ = 取覆盖对应的子群、$$G$$ = 由子群构造覆盖。完全相同的推理给出"子群的共轭类 ↔ 连通覆盖的同构类"，其中正规子群对应 Galois 覆盖。

一个证明，两门学科。**这正是本书把 Galois 理论放在最后的理由**：它不是一块新领地，而是那条从第 16 章就开始的"拓扑 ↔ 代数"对应线的终点站。

### 3.5 三角：几何、物理、代数在同一行公式里

**为什么要专门算一遍 Maxwell 方程组。** 前四条共同定理都是纯代数/纯分析的陈述；这一条要展示同一个算子 $$d$$（$$d^2=0$$，3.2 节的那个 $$D$$）在物理里长什么样。第 79 章已经在 $$\mathbb R^3$$ 上手算过 $$d(f\,dx)$$、$$d(P\,dx+Q\,dy)$$ 这类低次的具体外微分；这里把同一个 $$d$$ 搬到 $$\mathbb R^4$$（多一个时间坐标）、作用在编码电磁场的 2-形式 $$F$$ 上，逐分量算出 $$dF=0$$ 到底是哪几条物理定律——答案是：**齐次 Maxwell 方程组不是额外的物理假设，而是 $$d^2=0$$ 这一条纯代数事实的直接推论**。
...
在 (i) 中取 $$Z=P$$、$$f=\pi_X$$、$$g=\pi_Y$$：满足同样条件的 $$h$$ 是**唯一**的，而 $$h=\mathrm{id}_P$$ 满足这两式（$$\pi_X\mathrm{id}_P=\pi_X$$、$$\pi_Y\mathrm{id}_P=\pi_Y$$），故 $$vu=\mathrm{id}_P$$。

**（展开：对称地得到 $$uv=\mathrm{id}_{P'}$$）** 用结合律计算 $$uv:P'\to P'$$：
$$\pi'_X(uv)=(\pi'_Xu)v=\pi_Xv=\pi'_X,\qquad \pi'_Y(uv)=(\pi'_Yu)v=\pi_Yv=\pi'_Y.$$
在 (i) 中取 $$Z=P'$$、$$f=\pi'_X$$、$$g=\pi'_Y$$：满足这两式的 $$h$$ 唯一，而 $$h=\mathrm{id}_{P'}$$ 显然满足，故 $$uv=\mathrm{id}_{P'}$$。于是 $$u$$ 是同构，且由万有性质的唯一性，这样的 $$u$$ 只有一个。
...
| 子群 $$H$$（按包含序，从小到大） | 固定域 $$K^H$$（按包含序，从大到小） | $$[K^H:\mathbb Q]$$ |
|---|---|---|
